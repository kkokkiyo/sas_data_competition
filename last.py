...
df_cust.drop(columns=['join_date_dt'], inplace=True)
...
# ========================================================
# 4. [모델 1] Churn 예측 (데이터 누수 방지: OOF 적용 💡)
# ========================================================
print("\n🌳 [1/2] 이탈(Churn) 예측 모델 학습 중 (5-Fold OOF 누수 방지 적용)...")
clf_churn = lgb.LGBMClassifier(random_state=42, n_estimators=500, learning_rate=0.05, verbose=-1)

# 💡 [핵심 수정] Train 데이터에 대한 이탈 확률을 뽑을 때, 자기가 학습하지 않은 폴드(조각)의
# 데이터를 예측하게 만들어(OOF) LTV 모델이 컨닝하지 못하게 '정직한 확률'을 뽑아냅니다.
train_churn_prob_oof = cross_val_predict(clf_churn, X_train, y_churn_train, cv=5, method='predict_proba')[:, 1]

# 전체 Train 데이터로 최종 학습 (이후 Val, Test 예측을 위함)
clf_churn.fit(X_train, y_churn_train)
pred_churn_val = clf_churn.predict_proba(X_val)[:, 1]

# ========================================================
# 5. [모델 2] LTV 예측 (정직한 확률을 이용한 체이닝)
# ========================================================
print("🌳 [2/2] LTV 예측 모델 학습 중 (정직한 OOF 확률 체이닝 결합)...")
y_ltv_train_log = np.log1p(y_ltv_train)

X_train_ltv = X_train.copy()
X_val_ltv = X_val.copy()

# 💡 이전에 과적합(100% 정답)되었던 확률 대신, OOF로 구한 현실적인 확률을 넣어줍니다.
X_train_ltv['pred_churn_prob'] = train_churn_prob_oof 
...
submission_filename = 'Fukuoka_submission_최종안전본.csv'
